...
public class ChapterUtils implements AppConstants{
    ...
    public static List<ItemDto> getSections(int chapterNum) {
        ...
        if(chapterNum == 1) {
            ...
            items.add(new ItemDto(2,CHAPTER_1TH_PGM_02,CHAPTER_1TH_PGM_02));
        }
        if(chapterNum == 2) {
    ...
    public static void initLoadOpenCV(Context context){
        boolean success = OpenCVLoader.initDebug();//加载动态库
        if(success){
            Log.i(CV_TAG, "OpenCV加载完成");
        }else{
            Toast.makeText(context,"无法加载OpenCV", Toast.LENGTH_LONG).show();
        }
    }
}
